...
function resolveRange(filters, sprints, config) {
  ...
  switch (filters.preset) {
    ...
    case "last_week": {
      ...
      return {
        start: last.startOf("isoWeek").format("YYYY-MM-DD"),
        end: last.endOf("isoWeek").format("YYYY-MM-DD"),
        label: "Last Week",
      };
    }
    case "current_sprint":
    ...
    case "last_month": {
      ...
      return {
        start: last.startOf("month").format("YYYY-MM-DD"),
...
function resolveAutomationTotals({
  uiAutomated,
  apiAutomated,
  inSprintAutomated,
  backlogAutomated,
...
function moduleSnapshot(mod, allUpdates, config) {
...
function overallKpis(modules, updates, config) {
  ...
  const inSprintAutomated = snapshots.reduce((a, m) => a + m.current.inSprintAutomated, 0);
  const backlogAutomated = snapshots.reduce((a, m) => a + m.current.backlogAutomated, 0);
  return {
    totalTestCases,
    manualWritten,
    uiAutomated,
...
function inSprintVsBacklog(updates, today = dayjs()) {
  ...
  return {
    inSprint: {
      today: sumField(todayRows, "inSprintAutomated"),
      week: sumField(weekRows, "inSprintAutomated"),
      cumulative: sumField(updates, "inSprintAutomated"),
    },
    backlog: {
      today: sumField(todayRows, "backlogAutomated"),
      week: sumField(weekRows, "backlogAutomated"),
      cumulative: sumField(updates, "backlogAutomated"),
    },
  };
}
...
function sprintProgress(sprints, updates, modules, config) {
  return sprints.map((sprint) => {
    ...
    return {
      ...
      manualPct,
      executionPct,
      status: coverageStatus(completion, config.thresholds),
    };
  });
}

function weeklyStatus(updates, weekStart) {
  ...
  const previousWeek = {
    start: prev.startOf("isoWeek").format("YYYY-MM-DD"),
    end: prev.endOf("isoWeek").format("YYYY-MM-DD"),
  };
...
function bimonthly(modules, updates, startDate, endDate, config) {
...
module.exports = {
  ...
  sumField,
  dailyTotal,
  computeTotalAutomated,
  resolveAutomationTotals,
  moduleSnapshot,
  overallKpis,
  periodTotals,
  dailyTrend,
